...
namespace ObsAgent
{
    public sealed class ObsAgentOperations
    {
        ...
        public ObsAgentOperations(
            Func<ObsAgentConfiguration> configProvider,
            Action<string> log )
        {
        ...
        {
            return ExecuteSimpleRequestAsync(
                "GetVersion",
                "OBS WebSocket 연결에 성공했습니다.",
                cancellationToken );
        }

        public Task<AgentApiResponse> StartRecordAsync(
        ...
        public Task<AgentApiResponse> StopRecordAsync(
            CancellationToken cancellationToken )
        {
            return ExecuteSimpleRequestAsync(
                "StopRecord",
        ...
        public Task<AgentApiResponse> StartStreamAsync(
            CancellationToken cancellationToken )
        {
            return ExecuteSimpleRequestAsync(
                "StartStream",
        ...
        public Task<AgentApiResponse> StopStreamAsync(
            CancellationToken cancellationToken )
        {
            return ExecuteSimpleRequestAsync(
                "StopStream",
        ...
        public async Task<AgentApiResponse> SetSceneAsync(
            string sceneName,
            CancellationToken cancellationToken )
        {
            if( string.IsNullOrWhiteSpace( sceneName ) )
            {
                return AgentApiResponse.Error(
                    "sceneName이 비어 있습니다.",
                    IsObsRunning() );
            }
        ...
        private async Task<AgentApiResponse> ExecuteSimpleRequestAsync(
            string requestType,
        ...
        private static async Task SetSceneAsync(
        ...
        private ObsWebSocketClient CreateWebSocketClient(
            ObsAgentConfiguration config )
        {
            return new ObsWebSocketClient(
                "127.0.0.1",
                config.obsWebSocketPort,
                config.obsWebSocketPassword );
        }

        private async Task<bool> EnsureObsStartedAsync(
            ObsAgentConfiguration config,
            CancellationToken cancellationToken )
        {
            if( IsObsRunning() )
                    ...
                    config.obsExecutablePath ) )
            {
                throw new InvalidOperationException(
                    "OBS 실행 파일 경로가 비어 있습니다." );
            }

            if( !File.Exists( config.obsExecutablePath ) )
            {
                throw new FileNotFoundException(
                    "OBS 실행 파일을 찾을 수 없습니다.",
                    config.obsExecutablePath );
            }

            string workingDirectory =
                Path.GetDirectoryName(config.obsExecutablePath);

            if( string.IsNullOrWhiteSpace( workingDirectory ) )
            {
                throw new InvalidOperationException(
                    "OBS Working Directory를 확인할 수 없습니다." );
            }
            ...
            bool ready = await WaitForPortAsync(
                "127.0.0.1",
                config.obsWebSocketPort,
            ...
            if( !ready )
            {
                throw new TimeoutException(
                    "OBS는 실행되었지만 WebSocket 서버가 준비되지 않았습니다. " +
