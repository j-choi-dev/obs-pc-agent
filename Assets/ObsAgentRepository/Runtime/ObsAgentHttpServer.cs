...
namespace ObsAgent
{
    public sealed class ObsAgentHttpServer
    {
        ...
        public ObsAgentHttpServer(
            Func<ObsAgentConfiguration> configProvider,
            ObsAgentOperations operations,
            Action<string> log )
        {
        ...
        public void Start()
        {
                ...
                config.listenPort > 65535 )
            {
                throw new InvalidOperationException(
                    "Agent 포트는 1024~65535 범위로 설정하세요." );
            }
            ...
            _acceptLoop = Task.Run(
                () => AcceptLoopAsync( _cancellation.Token ) );

            _log(
                $"Agent HTTP 서버 시작: " +
                $"{bindAddress}:{config.listenPort}" );
        }
            ...
            CancellationToken cancellationToken )
        {
            ...
            if( !IsAuthorized( request ) )
            {
                await WriteJsonAsync(
                    ...
                    401,
                    AgentApiResponse.Error(
                        "Authorization Bearer Token이 올바르지 않습니다.",
                        _operations.IsObsRunning() ),
                    cancellationToken );
            ...
            switch( path )
            {
                case "/api/obs/launch":
                    response =
                        await _operations.LaunchObsAsync(
                            cancellationToken );
                    break;

                case "/api/obs/test":
                    response =
                        await _operations.TestConnectionAsync(
                            cancellationToken );
                    break;

                case "/api/obs/record/start":
                    response =
                        await _operations.StartRecordAsync(
                            cancellationToken );
                    break;

                case "/api/obs/record/stop":
                    response =
                        await _operations.StopRecordAsync(
                            cancellationToken );
                    break;

                case "/api/obs/stream/start":
                    response =
                        await _operations.StartStreamAsync(
                            cancellationToken );
                    break;

                case "/api/obs/stream/stop":
                    response =
                        await _operations.StopStreamAsync(
                            cancellationToken );
                    break;

                case "/api/obs/scene":
                    SceneCommandRequest command =
                        ParseSceneCommand(request.Body);

                    response =
                        await _operations.SetSceneAsync(
                            command.sceneName,
                            cancellationToken );
                    break;
        ...
        private static SceneCommandRequest ParseSceneCommand(
            string body )
        {
            if( string.IsNullOrWhiteSpace( body ) )
            {
                throw new InvalidOperationException(
                    "장면 변경 요청 본문이 비어 있습니다." );
            }

            SceneCommandRequest command =
                JsonUtility.FromJson<SceneCommandRequest>(body);

            if( command == null ||
                string.IsNullOrWhiteSpace( command.sceneName ) )
            {
                throw new InvalidOperationException(
                    "sceneName이 필요합니다." );
            }
        ...
        private static HttpRequestData ParseHeaders(
            string headersText )
        {
        ...
        private static async Task WriteEmptyAsync(
            NetworkStream stream,
            int statusCode,
            CancellationToken cancellationToken )
        {
